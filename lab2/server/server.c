...
/*A frame packet with unique id, length and data to transfer block by block*/
struct frame_t {
	long int id;
	long int length;
	char data[BUF_SIZE];
...
/*
ls -> This function scans the current directory and updates the input file with a complete list of 
 files and directories present in the current folder.
	f->	Input file which will be updated with the list of current files and 
...
int main(int argc, char **argv)
{
	...
	ssize_t length;
	off_t f_size; 	
	long int ack_num = 0;    //frame acknowledgement
	int ack_send = 0;
	int sfd;
	...
	sv_addr.sin_port = 8080;
	sv_addr.sin_addr.s_addr = INADDR_ANY; //localhost
...
/*----------------------------------------------------------------"get case"---------------------------------------------------------------*/
		...
		if ((strcmp(cmd_recv, "get") == 0) && (flname_recv[0] != '\0')) {
			...
			if (access(flname_recv, F_OK) == 0) {			//Check if file exist
				...
				{
// Handling of  failures in the message transmission
